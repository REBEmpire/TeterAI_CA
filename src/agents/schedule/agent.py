"""
ScheduleReviewAgent — processes ASSIGNED_TO_AGENT tasks with document_type == SCHEDULE_REVIEW.

Pipeline:
  1. Transition task → PROCESSING
  2. Fetch email ingest + extract document text
  3. Call ScheduleExtractor (EXTRACT capability)
  4. Call ScheduleReviewer (REASON_STANDARD + RED_TEAM_CRITIQUE)
  5. Store initial_review, red_team_critique, final_output to Firestore schedule_reviews/{task_id}
...
from .reviewer import ScheduleReviewer, write_schedule_review
...
class ScheduleReviewAgent:
    def __init__(self, gcp: GCPIntegration, ai_engine: AIEngine):
        self._gcp = gcp
        self._extractor = ScheduleExtractor(ai_engine)
        self._reviewer = ScheduleReviewer(ai_engine)

    def run(self) -> list[str]:
        """Process all ASSIGNED_TO_AGENT schedule review tasks. Returns list of processed task_ids."""
        if not self._gcp.firestore_client:
            logger.error("Firestore client not available — aborting Schedule Review Agent run.")
            return []
        ...
        for doc in assigned:
            ...
            logger.info(f"[{task_id}] Schedule Review Agent picking up task.")
    ...
    def _process_task(self, db, task_id: str, ingest_id: str, project_id: str) -> None:
        ...
        self._transition(db, task_id, "ASSIGNED_TO_AGENT", "PROCESSING",
                         "Schedule Review Agent started.", now)
        ...
        try:
            ...
            logger.error(f"[{task_id}] Schedule review failed: {e}")
            self._set_error(db, task_id, str(e))
            return

        # Step 5: Store to Firestore schedule_reviews
        try:
            write_schedule_review(db, task_id, project_id, review_result)
        ...
        try:
            ...
            history.append({
                ...
                "notes": "Schedule review complete — awaiting owner review.",
            })
