"""
CostAnalyzerAgent — processes ASSIGNED_TO_AGENT tasks with document_type == COST_ANALYSIS.

Pipeline:
  1. Transition task → PROCESSING
  2. Fetch email ingest + extract document text
  3. Call CostExtractor (EXTRACT capability)
  4. Call CostReviewer (REASON_STANDARD + RED_TEAM_CRITIQUE)
  5. Store initial_review, red_team_critique, final_output to Firestore cost_analyses/{task_id}
...
from .reviewer import CostReviewer, write_cost_analysis
...
class CostAnalyzerAgent:
    def __init__(self, gcp: GCPIntegration, ai_engine: AIEngine):
        self._gcp = gcp
        self._extractor = CostExtractor(ai_engine)
        self._reviewer = CostReviewer(ai_engine)

    def run(self) -> list[str]:
        """Process all ASSIGNED_TO_AGENT cost analysis tasks. Returns list of processed task_ids."""
        if not self._gcp.firestore_client:
            logger.error("Firestore client not available — aborting Cost Analyzer Agent run.")
            return []
        ...
        for doc in assigned:
            ...
            logger.info(f"[{task_id}] Cost Analyzer Agent picking up task.")
    ...
    def _process_task(self, db, task_id: str, ingest_id: str, project_id: str) -> None:
        ...
        self._transition(db, task_id, "ASSIGNED_TO_AGENT", "PROCESSING",
                         "Cost Analyzer Agent started.", now)
        ...
        try:
            ...
            logger.error(f"[{task_id}] Cost review failed: {e}")
            self._set_error(db, task_id, str(e))
            return

        # Step 5: Store to Firestore cost_analyses
        try:
            write_cost_analysis(db, task_id, project_id, review_result)
        ...
        try:
            ...
            history.append({
                ...
                "notes": "Cost analysis complete — awaiting owner review.",
            })
